...
# https://sourceforge.net/projects/icu/files/ICU4C/
...
RUN set -xe; \
    ...
    apk add --no-cache --repository http://dl-cdn.alpinelinux.org/alpine/edge/community/ --allow-untrusted \
        gnu-libiconv; \
    curl -sS -o /tmp/icu.tar.gz -L https://freefr.dl.sourceforge.net/project/icu/ICU4C/${ICU_VERSION}/icu4c-$(set +x && echo $ICU_VERSION | tr '.' '_' && set -x)-src.tgz; \
